...
export default class AdminController implements IAdminController {
  ...
  async getReportsData(
    ...
    try {
      ...
      logger.debug(
        `Received query params - pageNo: ${pageNo}, rowsPerPage: ${rowsPerPage}`
      );
      const reportsData = await this.adminServices.getReportsData(
        Number(pageNo),
        Number(rowsPerPage)
      );
  ...
  async getDashboardCardData(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      ...
      logger.error(`Error in getDashboardCardData: ${error.message}`, {
        error,
      });
      next(error);
    }
