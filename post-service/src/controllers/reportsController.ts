...
export default class ReportsController implements IReportsController {
  ...
  async addReport(req: any, res: Response, next: NextFunction) {
    try {
      ...
      logger.info(
        `Received report for ${entityType} with ID: ${entityId} by user: ${reportedBy}`
      );
      ...
      logger.info(
        `Report successfully added for ${entityType} with ID: ${entityId}`
      );
      res.status(StatusCode.OK).send(reportData);
    } catch (error: any) {
      logger.error(
        `Error in adding report for entity: ${req.params.entityType} with ID: ${req.params.entityId}. Error: ${error.message}`
      );
      next(error);
    }
  }
