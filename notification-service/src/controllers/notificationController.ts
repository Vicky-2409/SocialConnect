...
export default class NotificationController implements INotificationController {
  ...
  async getNotifications (
    ...
    try {
      ...
      const notification = await this.notificationServices.getNotifications(userId);
      logger.info(`Successfully fetched notifications for user ID: ${userId}`);
      res.status(StatusCode.OK).json({ message: MESSAGES.NOTIFICATIONS_FETCH_SUCCESS, data: notification });
    } catch (error: any) {
      logger.error(`Error while fetching notifications for user ID: ${req.user._id}. Error: ${error.message}`);
      next(error);
    }
  }
}
