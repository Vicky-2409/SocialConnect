...
function Feed({ currUserData }: { currUserData?: IUser }) {
  ...
  useEffect(() => {
    (async function () {
      try {
        ...
        setDisplayPosts(
          mergePosts(
            filteredPosts.slice(0, POSTS_PER_PAGE),
            promotedPosts || []
          )
        );
        ...
        setDisplayOtherPosts(
          mergePosts(otherPosts.slice(0, POSTS_PER_PAGE), promotedPosts || [])
        );
      } catch (error: any) {
  ...
  const loadMorePosts = useCallback(() => {
    if (filteredPosts) {
      const startIndex =
        (repeatCount.current * POSTS_PER_PAGE) % filteredPosts.length;
      let newPosts = [];
      ...
      if (startIndex + POSTS_PER_PAGE > filteredPosts.length) {
        newPosts = [
          ...
          ...filteredPosts.slice(
            0,
            POSTS_PER_PAGE - (filteredPosts.length - startIndex)
          ),
        ];
      } else {
      ...
      setDisplayPosts((prev) => [
        ...prev,
        ...mergePosts(newPosts, promotedPosts || []),
      ]);
  ...
  const loadMoreOtherPosts = useCallback(() => {
    if (otherPosts) {
      const startIndex =
        (repeatCount.current * POSTS_PER_PAGE) % otherPosts.length;
      let newPosts = [];
      ...
      if (startIndex + POSTS_PER_PAGE > otherPosts.length) {
        newPosts = [
          ...
          ...otherPosts.slice(
            0,
            POSTS_PER_PAGE - (otherPosts.length - startIndex)
          ),
        ];
      } else {
