...
const AddCommentForm: React.FC<CommentFormProps> = ({ userData, userId }) => {
  ...
  const onSubmit = useCallback(
    async (data: FormInputs) => {
      ...
      try {
        ...
        if (newComment && newComment._id) {
          setNewComments((comments) => [newComment, ...comments]);
          reset();
        } else {
          toast.error("Unexpected response structure");
        }
      } catch (error: any) {
        toast.error(error.message || "An error occurred while adding your comment");
      } finally {
        setIsSubmitting(false);
      }
    },
    [id, reset, isSubmitting]
  );
  ...
  return (
    <div className="max-w-3xl mx-auto mt-2">
      {/* Comment Form */}
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 transition-all duration-300 hover:shadow-md mb-8">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6">
          <div className="flex gap-4 items-start">
            {/* Profile Picture */}
            <div className="flex-shrink-0">
              <div className="relative w-10 h-10 overflow-hidden rounded-full ring-2 ring-gray-50">
                <Image
                  src={profilePicUrl}
                  alt="Profile"
                  fill
            ...
            <div className="flex-1 space-y-3">
              <div className="relative">
                <textarea
                  className="w-full min-h-[120px] p-4 rounded-xl bg-gray-50 border-0 focus:ring-2 focus:ring-blue-100 text-gray-700 placeholder-gray-400 text-sm resize-none transition-all duration-300"
