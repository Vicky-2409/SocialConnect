...
export default class ProfileController implements IProfileController {
  ...
  async editUser(req: any, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      try {
        logger.debug("Sending user data to message queues", {
          userId: userData._id,
        });
        ...
        logger.info("Successfully sent user data to message queues", {
          userId: userData._id,
        });
      } catch (error: any) {
  ...
  async updatePic(req: any, res: Response, next: NextFunction): Promise<void> {
    ...
    try {
      ...
      logger.info("WeNet tick request picture uploaded", {
        userId: req?.user?._id,
      });
      next();
    } catch (error) {
  ...
  async isFollowing(
    ...
    try {
      ...
      const currentUserId = req.user._id;
      logger.debug("Checking following status", {
        currentUserId,
        targetUserId: userId,
      });
  ...
  async toggleBlockUser(
    ...
    try {
      ...
      logger.debug("Processing block toggle", {
        currUserId,
        targetUserId: userId,
      });
  ...
  async isBlocked(req: any, res: Response, next: NextFunction): Promise<void> {
    try {
      const { userId } = req.params;
      const currentUserId = req.user._id;
      logger.debug("Checking block status", {
        currentUserId,
        targetUserId: userId,
      });
